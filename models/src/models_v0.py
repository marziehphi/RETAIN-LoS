...
class LiteLSTM(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        scheduler = {"scheduler": scheduler, "interval": "step", "monitor": "train_loss_step"}
    ...
    def training_epoch_end(self, outputs):
        ...
        for key, value in log_dict.items():
            self.log(f"train_{key}", value, on_epoch=True, prog_bar=True, logger=True)
    ...
    def step_epoch_end(self, outputs):
        _y_hat_los, _y_los = [], []
        _y_hat_mort, _y_mort = [], []
        for output in outputs:
            if "_y_hat_los" in output:
