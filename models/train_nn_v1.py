import torch
import numpy as np
from nn_utils_v1 import DataGenerator, save_model
...
'''
train_nn_v1.py written to train on 4 chosen model for the _eicu_features.csv from _preprocess_eICU.py.
...
parser = argparse.ArgumentParser(description='Code to train RNN and LSTM models')
...
data_generator = DataGenerator(args.path, args.batch_size, args.ignore_time_series, args.use_first_record, args.use_last_record, mode='train', use_cuda=args.use_cuda)
model = ModelClass(329,embedding_size, hidden_size)
if args.use_cuda:
    model = model.cuda()
...
save_model(model, 'models_v_1', {'embedding_size': embedding_size, 'hidden_size': hidden_size, 'lr': args.lr, 'epochs': args.epochs, 'batch_size': args.batch_size, 'reversed': args.reverse_input, 'ignor_time_series': args.ignore_time_series, 'use_last_record': args.use_last_record, 'use_first_record': args.use_first_record})
